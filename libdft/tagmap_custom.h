...
typedef cpp11::array<tag_t, DIR_PAGE_SZ> tag_page_t;
typedef cpp11::array<tag_page_t*, DIR_TABLE_SZ> tag_table_t;
typedef cpp11::array<tag_table_t*, DIR_SZ> tag_dir_t;
...
inline void tag_dir_setb(tag_dir_t& dir, ADDRINT addr, tag_t const& tag)
{
	...
	if (dir[virt2table(addr)] == NULL)
	{
		...
		tag_table_t* new_table = new tag_table_t(); // DCD (nothrow)
		if (new_table == NULL)
		{
	...
	if ((*table)[virt2page(addr)] == NULL)
	{
		...
		tag_page_t* new_page = new tag_page_t(); // DCD (nothrow)
		if (new_page == NULL)
		{
			LOG("Failed to allocate tag page!\n");
			libdft_die();
		}
		cpp::fill(new_page->begin(), new_page->end(), tag_traits<tag_t>::cleared_val);
